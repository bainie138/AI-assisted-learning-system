...
if st.session_state.selected_note:
    # 显示笔记详情
    note = get_note_by_id(st.session_state.selected_note)
    if note:
        ...
        if st.session_state.edit_mode:
            ...
            col1, col2 = st.columns(2)
            with col1:
                if st.button("💾 保存修改", type="primary", disabled=not (edit_title and edit_content)):
                    # 更新数据库
                    update_note(
                        note['id'],
                        title=edit_title,
        ...
        with col1:
            ...
            if not st.session_state.edit_mode:
                if st.button("✏️ 编辑笔记", key=f"edit-btn-{note['id']}"):
        ...
        with col2:
            # 删除按钮
            if not st.session_state.edit_mode:
                if st.button("🗑️ 删除笔记", key=f"delete-btn-{note['id']}"):
                    delete_note(note['id'])
                    st.session_state.selected_note = None
                    st.session_state.edit_mode = False
                    st.session_state.refresh_key += 1
        with col3:
            if st.button("🔄 刷新", key=f"refresh_btn_{note['id']}"):
                st.session_state.edit_mode = False
                st.session_state.edit_title = ""
                st.session_state.edit_content = ""
                st.session_state.refresh_key += 1
    ...
    # 保存按钮
    if st.button("💾 保存笔记", type="primary", disabled=not (title and content)):
        # 保存笔记（不自动调用AI）
        create_note(title, content, keywords=None, summary=None)
        st.success("🎉 笔记保存成功！")
